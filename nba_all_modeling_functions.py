...
# For ensembling
def nba_ensemble(df, list_of_feature_columns, target_feature, k=10, print_all=False, plot=False):
    '''
    Parameters:
    -----------
    ...
    list_of_models = [("logr", LogisticRegression()), (f'KNN {k}', KNeighborsClassifier(n_neighbors=k, n_jobs=-1)), ("ran_for", RandomForestClassifier()), ("svc_model", SVC(probability=True))]
...
def nba_ensemble_predict(df, list_of_feature_columns, target_feature, max_age=50, k=9, print_all=False, plot=False):
    ...
    scaler, MODEL = nba_ensemble(df, list_of_feature_columns, target_feature, k=10, print_all=print_all, plot=False)
